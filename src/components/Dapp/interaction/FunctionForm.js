...
const FunctionForm = ({
  ...
  const callBlockchainMethod = async (_props) => {
    ...
    const inputs = [];
    const sortedProps = Object.keys(_props)
      .sort()
      .forEach((key) => inputs.push(_props[key]));
  ...
  const callPayableFunction = async (_props) => {
    ...
    signTransaction(contract._address, normalizedId, Object.values(_props))
      .then((result) => {
        const endTime = new Date();
        // console.log('** [TIME REQ  (' + normalizedId + ')]:', endTime);
        // console.log(
        //   '** [TIME REQ  (' + normalizedId + ')] - Elapsed Time :',
        //   endTime - startTime,
        // );
  ...
  function getFunctionName() {
    ...
    if (
      ...
      element.element.businessObject &&
      element.element.businessObject.extensionElements.values
    ) {
  ...
  return (
    <div style={{ border: '1px solid black', padding: '20px 40px 20px 40px' }}>
      ...
      <div>
        <strong>SID:</strong> {_sid}
      </div>
      <div>
        <strong>Participant:</strong> <Tag color="blue">{Pname}</Tag>
      </div>
      {/*{getFunctionName() &&*/}
      ...
      <Form onSubmit={handleSubmit}>
        {currentElement &&
          ...
          currentElement.inputs.length > 0 && (
            <div
              style={{
                marginTop: '10px',
            ...
            >
              {currentElement.inputs.map((currentBpmnElement, index) => {
                ...
                if (
                  currentBpmnElement.name.includes('_rcid') ||
                  currentBpmnElement.name.includes('_rcbkid')
                ) {
                  ...
                  return (
                    <div key={index}>
                      <IPFSForm
                        onOk={(value) => value && setIpfsId(value)}
                        sid={sid}
                      />
                    </div>
                  );
                ...
                return (
                  <Form.Item key={index} label={currentBpmnElement.name}>
        ...
        <div style={{ textAlign: 'center' }}>
          <Button htmlType="submit" type="primary" disabled={!ipfsContent}>
            Send
          </Button>
        </div>
        {/*</Form.Item>*/}
...
const WrappedFunctionForm = Form.create({ name: 'function_form' })(
  FunctionForm,
);
...
function inputSelector(form, current) {
  ...
  switch (current.type) {
    ...
    default:
      ...
      })(<Input placeholder={`${current.type} ${current.name}`} />);
  }
}
...
function InputUint256({ form, current }) {
  return form.getFieldDecorator(current.name, {
    rules: [
      { required: true, message: 'Please fill the field ' + current.name },
    ],
  })(<InputNumber placeholder={`${current.type} ${current.name}`} />);
}
