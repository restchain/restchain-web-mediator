...
import { ipfsAdd, ipfsAddWithLog } from '../ipfsClient';

const { TextArea } = Input;

export function IPFSForm({ onOk, sid }) {
  ...
  const handleAdd = () => {
    ...
    // ipfsAdd(buffer)
    ipfsAddWithLog(value, sid).then((response) => {
      ...
      {!loading && !cid && (
        <>
          ...
          <TextArea
            rows={5}
            placeholder={
              '{ "para1": "string_value", "para2" : number_value, "para3": { "p1" : "string_value" }'
            }
